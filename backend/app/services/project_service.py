...
class ProjectService:
    ...
    async def create_project(self, client_id: UUID, project_data: ProjectCreate) -> Project:
    ...
    async def get_projects_by_client(self, client_id: UUID, skip: int = 0, limit: int = 20) -> List[Project]:
        """Get projects by client ID."""
        return self.project_repository.get_projects_by_client(client_id, skip=skip, limit=limit)

    async def get_projects_by_status(self, status: ProjectStatus, skip: int = 0, limit: int = 20) -> List[Project]:
        """Get projects by status."""
        return self.project_repository.get_projects_by_status(status, skip=skip, limit=limit)

    async def get_projects_by_professional(self, professional_id: UUID, skip: int = 0, limit: int = 20) -> List[Project]:
        """Get projects by professional ID."""
        return self.project_repository.get_projects_by_professional(professional_id, skip=skip, limit=limit)
    ...
    async def assign_professional(self, project_id: UUID, professional_id: UUID) -> Optional[Project]:
        ...
        if project.status != ProjectStatus.OPEN:
            raise ValueError(f"Project is not open for assignment. Current status: {project.status}")
    ...
    async def update_status(self, project_id: UUID, status: ProjectStatus) -> Optional[Project]:
        ...
        if project.status == ProjectStatus.COMPLETED and status != ProjectStatus.COMPLETED:
            raise ValueError("Cannot change status of completed project")

        if project.status == ProjectStatus.CANCELLED and status != ProjectStatus.CANCELLED:

...
async def list_reviews(
    ...
    try:
        if project_id:
            ...
            reviews = await review_service.get_reviews_by_project(project_uuid, skip=skip, limit=limit)
        elif reviewer_id:
            reviewer_uuid = UUID(reviewer_id)
            reviews = await review_service.get_reviews_by_reviewer(reviewer_uuid, skip=skip, limit=limit)
        elif reviewed_id:
            reviewed_uuid = UUID(reviewed_id)
            reviews = await review_service.get_reviews_by_reviewed(reviewed_uuid, skip=skip, limit=limit)
        elif review_type:
            reviews = await review_service.get_reviews_by_type(review_type, skip=skip, limit=limit)
        else:

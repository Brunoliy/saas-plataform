...
async def get_current_user(
    ...
    try:
        print(f"DEBUG: current_user_id = {current_user_id!r}, type = {type(current_user_id)}")
        ...
        print(f"DEBUG: ValueError when converting to UUID: {e}, input was: {current_user_id!r}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid user ID format: {current_user_id!r}")
